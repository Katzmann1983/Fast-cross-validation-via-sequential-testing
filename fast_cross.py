...
class fast_cross_validation:
    ...
    def topConfigurations(P1,mean_performance,alpha,K):
        ...
        for k in range(1,len(sorted_list)):
            p=cochrans_q(sorted_list[0:k,:])                 #Cochrans_q test for finding the top configuration
            if p[1]<=alpha_2:
                break
        k=k+1
        for x in range(0,k):
    ...
    def parametersList(self, conf):
        ...
        parameters_list = np.zeros(algo_param_numbers)                    #List for sroting the parameters of algorithm 
        for c in range(0,configurations):
            params_iterate = []                                                 #Paramters list for each iteration
            clf = classifiers_CV.classifiers.random_forest_classifier()   #Choosen classifier for prediction
            param_dist = {                                                #Choose discret and continous values for parameters randomly
    ...
    def CSVT_main_loop(self,fold,wstop,configuration,alpha,beta):
        ...
        :return: none
        """
        performce_mean = np.empty([configuration,fold])                     #Matrix for storing Mean Performance
        ...
        for fd in range(1,fold+1):                               #To find the top performing configurations for fold fd
            ...
            for c in range(0,configuration):                     
                if isActive[c] == 1:
                    ...
                    print("index 1......",ind1)
                    print("index 2....",ind2)
                    x = self.train.values[ind1:ind2]                    #Range of dataset of train in current fold
                    z = self.y[ind1:ind2]                               #Range of dataset of prediction in current fold    
                    test_CV = self.train.drop(train.index[[ind1,ind2]])  #Rest of the data for testing
                    v = pd.DataFrame(self.y)                            
                    y_test = v.drop(v.index[[ind1,ind2]])                #prediction of test data
                    myarray = np.array(parameters_list[c]).tolist()
                    clf = classifiers_CV.classifiers.random_forest_classifier(n_estimators = myarray[0],max_depth = myarray[1],
                                                        max_features = myarray[2], min_samples_split = myarray[3], 
                    ...
                    performce_mean[c,fd-1] = roc_score                    #Mean performance of each configuration
                    for l in range(0,len(reds)):                        #convertion of XGboost prediction into binary form
                        if reds[l]>0.5:                    
                            reds[l]=1
                        else:
                            reds[l]=0
            ...
            top_configurations = self.topConfigurations(pp_matrix,performace_matrix,alpha,K)     #Find the top configurations
            ...
            for z in range(0,len(matrix_trace[0])):
                T = self.isFlopConfiguration(matrix_trace[:,z],fd,fold,beta,alpha)        #Checking each configuration whether its Flop or not
            ...
            print("trace_matrix\n",trace_matrix)                                                               
            p = self.similarPerformance(trace_matrix[isActive_index[0],(f-wstop + 1):f],alpha)
